# How to build up a new DSIDE Midnight container
...
nmcli connection add type ethernet con-name \
      host-br0 ifname eth1 autoconnect yes \
      ipv4.addresses 192.168.0.222/24 ipv4.method manual
nmcli connection up host-br0
```

## Install latest CMAKE in container from source
```
# Needed for now 
# (rocky doesn't come with new enough cmake to build sigma and friends)
...
## Update `.bashrc`
Need to add these environment variables the MTS knows what we are talking
```
cat >> ~/.bashrc << EOF

# Midnight Env Variables
...
_(see latest sigma `readme.md`)_
Build a new sigma for rocky in the container
